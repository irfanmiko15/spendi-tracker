...
class LoginViewModel {
    ...
    init(loginUseCase: LoginUseCase = LoginUseCase(repository: LoginRepositoryImpl(remoteDataSource: LoginRemoteDatasourceImpl()))) {
    ...
    func login(data: LoginRequestModel) async {
        ...
        do {
            ...
            let result = try await loginUseCase.execute(data: LoginRequestModel(email: data.email, password: data.password))
            if let res = result{
